...
void Version(byte locX, byte locY){
  ...
  lcd.print(F(" v2.12""\xE0"));
  lcd.write(7);
}

void Intestazione(){
  lcd.setCursor(0,0);
  lcd.print(F("BREWBOARD"));
  Version(12,0);
}
...
void LCD_QQSO(){
  lcd.setCursor(1,3);
  lcd.print(F("OMH OML Oversl  Ok"));
}
void LCD_QQxO(){
  lcd.setCursor(1,3);
  lcd.print(F(" OMH OML   ---  Ok"));
}
void LCD_SGEO(){
  lcd.setCursor(1,3);
  lcd.print(F("OMH  OML  Terug Ok"));
}
void LCD_xGEO(){
  lcd.setCursor(1,3);
  lcd.print(F("---  OML  Terug Ok"));
}

void LCD_SxEO(){
  lcd.setCursor(1,3);
  lcd.print(F("OMH  ---  Terug Ok"));
}
...
void Stage(byte Stage, float Set, float Temp){
  ...
  lcd.setCursor(1, 3);
  if (Stage ==  0)               lcd.print(F("OMH OML Pauze ---"));
  if (Stage  >  0 && Stage  < 8) lcd.print(F("OMH OML Pauze STP"));
  if (Stage ==  8)               lcd.print(F("OMH OML  ---  ---"));
  if (Stage ==  9)               lcd.print(F("OMH OML  ---  Pmp"));
  if (Stage == 10)               lcd.print(F("---* *--- ---  Pmp"));
  if (Stage == 11)               lcd.print(F("---* *--- Tijd Pmp"));
}
...
void PidSet(int pidSet, byte i){
  if (i == 0) {
    ...
    if (pidSet == 0) lcd.print(F("Electrsch "));
...
void UnitSet(byte unitSet, byte i){
  ...
  switch(i){
    ...
    case(1)://Sensore
      if (unitSet == 0) lcd.print(F(" Binnen"));
      else              lcd.print(F(" Buiten"));
      break;
 ...
 */
    ...
    case(17):
      if (unitSet == 0) lcd.print(F("    Uit"));
      if (unitSet == 1) lcd.print(F("   Koud"));
      if (unitSet == 2) lcd.print(F("   Warm"));
      break;
  }
}
...
void Menu_3_4_3(){
  lcd.setCursor(2,2);
  lcd.print(F("Recept verwijderen"));
  LCD_SGEO();
}
void Menu_3_4_4(){
  lcd.setCursor(2,2);
  lcd.print(F("Initializeren      "));
  LCD_SxEO();
}
...
void TemperaturaRaggiunta(){
  LCDClear(2);
  lcd.setCursor(3,2);
  lcd.print(F("Temp. Bereikt"));

  lcd.setCursor(1,3);
  lcd.print(F("Doorgaan: Ja  --- "));
}
